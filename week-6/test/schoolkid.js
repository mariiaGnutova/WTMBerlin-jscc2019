...
test('Create new schoolkid', async t => {
  ...
  test('Fetch a schoolkid', async t => {
    ...
    const fetchRes = await request(app).get(`/schoolkid/${mariiaUserCreated._id}`)
    // checking for server response status success
    // this endpoint is rendering into HTML
    t.is(fetchRes.status, 200)
  
    const fetchResJson = await request(app).get(`/schoolkid/${mariiaUserCreated._id}/json`)
    // checking for server response status success
  ...
  test('Delete a schoolkid', async t => {
    ...
    // first create a schoolkid
    const schoolkidToCreate = { name: 'Sergey',
    surname: 'Ivanov',
    gender: 'Herr',
    grades:[],
    parents: [] }
    ...
    const deleteRes = await request(app).delete(`/schoolkid/${sergeyUserCreated._id}`)
    // checking for server response status success
    ...
    const fetchJson = await request(app).get(`/schoolkid/${sergeyUserCreated._id}/json`)
    // checking for server response status - page not found 404
